...
def _localise_board_thread() -> None:
    ...
    try:
        ...
        if _LOCALISE_USE_J0_SWEEP:
            ...
            # Sobol low-discrepancy sampling within the configured (d, ev,
            # az) ranges — same continuous approach as the rest of the
            # codebase. Pick the most-vertical IK-feasible result.
            ...
            try:
                ...
                _post_status(
                    f"Localise: seed-pose generation raised {type(e).__name__}: {e}"
                )
                return
            if not seed_cands:
                ...
                _post_status(
                    ...
                    f"{_LOCALISE_SEED_TARGET_M} from {_LOCALISE_SEED_N_CANDIDATES} "
                    f"Sobol candidates within d∈{_LOCALISE_SEED_DISTANCE_RANGE_M} m, "
                    f"ev∈{_LOCALISE_SEED_ELEVATION_RANGE_DEG}°. "
                    f"Rejections: {rej}. Try widening the search ranges or "
                    "moving _LOCALISE_SEED_TARGET_M."
                )
                return
            ...
            logger.info(
                "localise (J0-sweep): seed search %d/%d reachable, "
                "chose vertical-score=%.3f, joints (deg)=%s",
