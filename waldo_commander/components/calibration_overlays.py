...
# overhead pose at workspace centre, then sweep just the base joint (J0)
# while keeping J1-J5 fixed. The camera gaze rotates in a circular arc
# around the base, scanning the entire forward workspace in one motion.
# Each J0 step captures a frame; detection successes are aggregated for
# median consensus. Set _LOCALISE_USE_J0_SWEEP=False to fall back to
# the multi-target scan.
_LOCALISE_USE_J0_SWEEP: bool = True
# Workspace centre to use as the seed pose's look-at target. Chosen to
# be in the FRONT of the robot (positive X) where reachability is best
# and the workspace lies geometrically.
_LOCALISE_SEED_TARGET_M: tuple[float, float] = (0.30, 0.0)
# Camera (distance, elevation) candidates for the seed pose, tried in
# order — first IK-feasible combo wins. Pinokin's IK feasibility against
# the tilt_x=180 mount is finicky and non-monotonic in elevation
# (e.g. elev=75° works but elev=80° fails at the same distance), so we
# enumerate several pairs instead of just one. Each pair is sampled
# across the full 360° azimuth — the wrist-flip kinematics make the
# reachable azimuths cluster at ±150°, NOT in the front (−90° to +90°),
# so a narrow front-only search misses every candidate.
_LOCALISE_SEED_PAIRS: tuple[tuple[float, float], ...] = (
    (0.24, 70.0),  # close + mostly-overhead — best vertical score in tests
    (0.24, 75.0),
    (0.28, 70.0),
    (0.28, 75.0),
    (0.24, 80.0),  # closer-overhead
    (0.32, 65.0),  # farther + more oblique fallback
    (0.24, 60.0),  # lower elevation, wider footprint fallback
)
...
def _localise_board_thread() -> None:
    ...
    try:
        ...
        if _LOCALISE_USE_J0_SWEEP:
            # J0-sweep mode: ONE seed pose at workspace centre + N J0 steps.
            # The seed pose is generated by the standard PoseGenerator
            # (so it respects all the workspace + IK constraints), then we
            # vary just the base joint to scan the workspace radially.
            seed_target = np.array(
                [_LOCALISE_SEED_TARGET_M[0], _LOCALISE_SEED_TARGET_M[1], scan_z],
                dtype=np.float64,
            )
            # Walk through (distance, elevation) candidate pairs until one
            # produces a reachable pose. Full 360° azimuth sweep per pair
            # because PAROL6's reachable cluster sits at ±150°, not in the
            # front hemisphere.
            seed_cands: list = []
            seed_chosen_pair: tuple[float, float] | None = None
            seed_last_rej: dict = {}
            for d_seed, ev_seed in _LOCALISE_SEED_PAIRS:
                seed_params = HemisphereParams(
                    distances_m=(d_seed,),
                    elevations_deg=(ev_seed,),
                    azimuth_counts=(16,),
                ...
                try:
                    cands_pair, stats_pair = seed_gen.generate(max_count=None)
                except Exception as e:  # noqa: BLE001
                    logger.warning(
                        "localise seed (d=%.2f, ev=%.0f) — generation raised %s: %s",
                        d_seed, ev_seed, type(e).__name__, e,
                    )
                    continue
                seed_last_rej = dict(getattr(stats_pair, "rejection_log", {}) or {})
                if cands_pair:
                    seed_cands = cands_pair
                    seed_chosen_pair = (d_seed, ev_seed)
                    logger.info(
                        "localise (J0-sweep) seed: %d reachable at d=%.2f m, ev=%.0f°",
                        len(cands_pair), d_seed, ev_seed,
                    )
                    break
            if not seed_cands:
                _post_status(
                    "Localise: no reachable seed pose at workspace centre "
                    f"{_LOCALISE_SEED_TARGET_M} across {len(_LOCALISE_SEED_PAIRS)} "
                    f"(d, elev) candidates. Last rejections: {seed_last_rej}. "
                    "Try widening _LOCALISE_SEED_PAIRS or moving "
                    "_LOCALISE_SEED_TARGET_M."
                )
                return
            seed_cands_sorted = sorted(
                seed_cands,
                key=lambda c: _vertical_score_for(c, seed_target),
                reverse=True,
            )
            seed_q_rad = np.asarray(
                seed_cands_sorted[0].joint_angles_rad, dtype=np.float64,
            )
            seed_q_deg = np.degrees(seed_q_rad)
            seed_j0_deg = float(seed_q_deg[0])
            logger.info(
                "localise (J0-sweep): chosen seed (d=%.2f, ev=%.0f), "
                "joint angles (deg) = %s, vertical-score = %.3f",
                seed_chosen_pair[0], seed_chosen_pair[1],
                ["%.1f" % v for v in seed_q_deg],
                _vertical_score_for(seed_cands_sorted[0], seed_target),
            )
            # Build candidates list: copies of the seed with J0 varied.
            # The pose generator returns CalibrationCandidate dataclasses;
            # we only need joint_angles_rad for the move_j call later, so
            # construct a lightweight stand-in.
            from dataclasses import dataclass as _dc  # noqa: PLC0415

            @_dc
            class _SeedCandidate:
                joint_angles_rad: NDArray[np.float64]
                flange_pose: NDArray[np.float64]
            ...
            for dj0 in j0_offsets:
                ...
                # Limit-check the J0 (other joints unchanged).
                if not scan_robot.check_limits(q_rad):
                    logger.debug(
                        "localise J0 sweep step dj0=%.1f° out of joint limits, skipping",
                        dj0,
                    )
                    continue
                # We don't strictly need flange_pose for the localise, but
                # downstream code expects the field; computing FK is cheap.
                fk_pose = np.zeros(6, dtype=np.float64)
                scan_robot.fk(q_rad, fk_pose)
                T_flange = np.eye(4, dtype=np.float64)
                from scipy.spatial.transform import Rotation as _R  # noqa: PLC0415
                T_flange[:3, :3] = _R.from_euler(
                    "XYZ", fk_pose[3:],
                ).as_matrix()
                T_flange[:3, 3] = fk_pose[:3]
                candidates.append(_SeedCandidate(
                    joint_angles_rad=q_rad,
                    flange_pose=T_flange,
                ))
            logger.info(
                "localise (J0-sweep): %d J0 steps queued (dj0 in [%.0f°, %.0f°], "
                "absolute J0 around %.1f°)",
                len(candidates),
                -_LOCALISE_J0_SWEEP_HALF_DEG, +_LOCALISE_J0_SWEEP_HALF_DEG,
                seed_j0_deg,
            )
            # Skip the multi-target loop below — already populated.
