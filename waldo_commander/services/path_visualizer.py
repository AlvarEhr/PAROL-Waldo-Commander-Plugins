...
def _run_simulation_isolated(
    ...
    """
    # Forward the GUI's active tool key into the subprocess env so
    # ``_check_segment_collision`` resolves it correctly. The cpu_bound
    # worker inherits env at fork time; the parent's call-time value wins.
    ...
    if gui_active_tool_key:
        ...
        # Clear stale value inherited from a prior pool run.
...
class PathVisualizer:
    ...
    async def update_path_visualization(
        ...
        async with self._simulation_lock:
            ...
            # Read selected_tool before cpu_bound: storage is request-
            # context-bound here but a stale on-disk snapshot inside the
            # worker.
            gui_active_tool_key = ""
            try:
